...
function getOffsets(prefix, index) {
...
export default function WhatIDoSection() {
  ...
  const services = useMemo(
    () => t("WhatIDo.services", { returnObjects: true }),
    [t],
  );

  const headerRefs = useRef([]);
  const [headerHeightsPx, setHeaderHeightsPx] = useState(() =>
  ...
  const measure = useCallback(() => {
    setHeaderHeightsPx(
      Array.from({ length: servicesLen }, (_, i) =>
        headerRefs.current[i]?.getBoundingClientRect().height ?? 0,
      ),
    );
  }, [servicesLen]);
  ...
  useLayoutEffect(() => {
    const nodes = headerRefs.current.filter(Boolean);
